...
export const API_BASE = 'http://localhost:8000' // TODO: Move to global .env file
...
export function verifyEmail(email: string, code: string) {
  return postJSON<{ status: 'verified' | 'ok' }>('/auth/verify-email', { email, code })
}
export function resendVerification(email: string) {
  return postJSON<{ status: 'ok' }>('/auth/resend-verification', { email })
}

export function loginDirect(email: string, password: string) { // TODO: Should probably be deleted
